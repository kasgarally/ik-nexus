...
NEXUS™ is INTELLEKTRA’s Meteor 3 + Vue 3 platform (governance, ERP, and related products). This monorepo holds **shared JavaScript packages**, **Meteor product apps**, and **non-JS services**. All original work is proprietary: [INTELLEKTRA © 2026](LICENSE), author Karmil Asgarally.
...
Three isolated install trees share one git repo. That is deliberate: Meteor 3 does not consume a hoisted pnpm workspace, and product apps pin Vue/Vuetify independently.
...
| Meteor products | [`apps/*`](apps/nexus-govrn) | GovRN today; later products copy the same `file:` + `registerWithMeteor` pattern |
| Non-JS | [`services/`](services), [`docker/`](docker) | XBRL/Arelle, Compose, NGINX, Mongo volumes |
...
Do not put Meteor apps or `services/` in the pnpm workspace.
...
Three layers: Vitest for `packages/*`, Meteor mocha for GovRN runtime (methods / HTTP / GridFS), Playwright for user-visible flows. Tests are not added during feature work unless a person asks. Commands, report paths, and CI: [`TESTING.md`](TESTING.md).
